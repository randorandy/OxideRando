...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"Oxide{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/Oxide.sfc"
    ...
    for boss in bosses :
        newmusic = random.choice(musics)
        romWriter.writeBytes(boss[1], newmusic)
        print(boss[0]," ",hex(boss[1])," ",str(newmusic))
        '''

    # Open Missile gives 575
    romWriter.writeBytes(0x260dd, b"\x3f\x02")
